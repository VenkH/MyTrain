...
export default defineComponent({
  ...
  setup () {
    ...
    const handleQuery = () => {
      loading.value = true;
      jobs.value = [];
      axios.get('/batch-service/admin/job/query').then((response) => {
        loading.value = false;
    ...
    const handleModalOk = () => {
      ...
      axios.post('/batch-service/admin/job/' + url, job.value).then((response) => {
    ...
    const handleDelete = (record) => {
      axios.post('/batch-service/admin/job/delete', {
        name: record.name,
        group: record.group
      }).then((response) => {
        const data = response.data;
    ...
    const handlePause = (record) => {
      axios.post('/batch-service/admin/job/pause', {
        name: record.name,
        group: record.group
      }).then((response) => {
        const data = response.data;
    ...
    const handleResume = (record) => {
      axios.post('/batch-service/admin/job/reschedule', record).then((response) => {
        modalLoading.value = false;
        const data = response.data;
        if (data.success) {
          modalVisible.value = false;
    ...
    const handleRun = (record) => {
      axios.post('/batch-service/admin/job/run', record).then((response) => {
        const data = response.data;
        if (data.success) {

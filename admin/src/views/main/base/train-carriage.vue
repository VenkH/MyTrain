  ...
  setup() {
    ...
    watch(() => trainCarriage.value.rowCount, ()=>{
      if (Tool.isNotEmpty(trainCarriage.value.rowCount) && Tool.isNotEmpty(trainCarriage.value.seatType)) {
        let colNum = COL_NUM_BY_TYPE_ARRAY[trainCarriage.value.seatType - 1].num;
    ...
    const onDelete = (record) => {
      axios.delete("/business-service/admin/train-carriage/delete/" + record.id).then((response) => {
    ...
    const handleOk = () => {
      axios.post("/business-service/admin/train-carriage/save", trainCarriage.value).then((response) => {
    ...
    const handleQuery = (param) => {
      ...
      loading.value = true;
      axios.get("/business-service/admin/train-carriage/query-list", {
        params: {
          page: param.page,
          size: param.size,

  ...
  <a-modal v-model:visible="visible" title="每日车站" @ok="handleOk"
    ...
    <a-form :model="dailyTrainStation" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="站名">
        <station-select-view v-model="dailyTrainStation.name"></station-select-view>
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onDelete = (record) => {
      axios.delete("/business-service/admin/daily-train-station/delete/" + record.id).then((response) => {
    ...
    const handleOk = () => {
      axios.post("/business-service/admin/daily-train-station/save", dailyTrainStation.value).then((response) => {
